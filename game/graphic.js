function init()
{
    ...
    enemy = new Player ("enemy", 0xffffff, new THREE.Vector2(-240, 157), 0)
...
function Ground(color, size_x, size_y, nb_tile)
{
    ...
    for (x = minX; x <= maxX; x = x+sizeOfTileX){
        for (y = minY; y <= maxY; y = y+sizeOfTileY){
            ...
            color = (x == sizeOfTileX && y == 0) ? colors[Math.floor(Math.random()*(colors.length-1))] : colors[Math.floor(Math.random()*colors.length)];
